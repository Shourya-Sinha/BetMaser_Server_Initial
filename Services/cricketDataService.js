...
//       // console.log("live matched data in server :- ",response);

//       if (response.data?.data) {
//         // Filter only live matches (not completed)
//         const liveMatches = response.data.data.filter(
//           match => match.matchStarted && !match.matchEnded
//         );
...
//   /**
//    * Fetch recent/completed cricket matches
//    */
//   static async getRecentMatches() {
//     try {
//       CLOG.info('Fetching recent cricket matches from CricAPI...');
...
//         CLOG.success(`Found ${upcomingMatches.length} upcoming matches`);
//         return upcomingMatches.slice(0, 15).map(match =>
//           this.formatMatch(match, 'upcoming')
//         );
//       }
//       return [];
//     } catch (error) {
...
//   static async getMatchScore(matchId) {
//     try {
//       CLOG.info(`Fetching match info for: ${matchId}`);
...
//       return null;
//     } catch (error) {
//       CLOG.error('CricAPI Match Info Error:', error.message);
//       return null;
//     }
//   }
...
//       return null;
//     } catch (error) {
//       CLOG.error('CricAPI Squad Error:', error.message);
//       return null;
//     }
//   }
...
//   /**
//    * Fetch all matches (live + upcoming + recent)
//    */
//   static async getAllMatches() {
...
//     CLOG.success(`Total unique cricket matches: ${uniqueMatches.length}`);
...
//     // Get team names and short names
//     const homeTeam = teams[0] || 'Team A';
//     const awayTeam = teams[1] || 'Team B';
//     const homeShort = teamInfo[0]?.shortname || homeTeam.substring(0, 4).toUpperCase();
//     const awayShort = teamInfo[1]?.shortname || awayTeam.substring(0, 4).toUpperCase();

//     // Format score
//     const scores = match.score || [];
//     const scoreDisplay = scores.map(s =>
//       `${s.inning?.split(' ')[0]}: ${s.r}/${s.w} (${s.o} ov)`
//     ).join(' | ');

//     // Determine actual status
//     let matchStatus = status;
//     if (match.matchStarted && !match.matchEnded) matchStatus = 'live';
//     else if (match.matchEnded) matchStatus = 'completed';
//     else if (!match.matchStarted) matchStatus = 'upcoming';
...
//   /**
//    * Format raw match data to our standard format
//    */
//   static formatMatch(match, status) {
//     const teams = match.teams || [];
//     const team1 = teams[0]?.team?.replace(/\s*\.\.\.?\s*/g, '').trim() || 'Team A';
//     const team2 = teams[1]?.team?.replace(/\s*\.\.\.?\s*/g, '').trim() || 'Team B';
//     const team1Score = teams[0]?.run || '';
//     const team2Score = teams[1]?.run || '';

//     return {
//       id: `cricket-${match.id}`,
//       apiId: match.id,
//       name: match.title || `${team1} vs ${team2}`,
//       type: 'cricket',
//       status: status,
//       teams: {
//         home: team1,
...
//         home: team1Score,
//         away: team2Score,
//         display: `${team1Score} vs ${team2Score}`,
//       },
//       overview: match.overview || '',
//       timeAndPlace: {
//         date: match.timeAndPlace?.date || 'TBD',
//         time: match.timeAndPlace?.time?.replace(/&nbsp;/g, ' ') || '',
//         place: match.timeAndPlace?.place || 'TBD',
//       },
//       venue: match.timeAndPlace?.place?.replace('at ', '') || 'TBD',
//       entryFee: 25,
//       prizePool: this.calculatePrizePool(status),
//       players: this.generateCricketPlayers(team1, team2),
//       timestamp: new Date().toISOString(),
//     };
//   }
...
//    */
//   static formatScoreData(data, matchId) {
//     return {
//       id: `cricket-${matchId}`,
//       title: data.title || '',
//       update: data.update || '',
...
//         balls: data.batsmanOneBall || '(0)',
//         strikeRate: data.batsmanOneSR || '0',
//       },
//       batsmanTwo: {
//         name: data.batsmanTwo || '',
//         runs: data.batsmanTwoRun || '0',
//         balls: data.batsmanTwoBall || '(0)',
//         strikeRate: data.batsmanTwoSR || '0',
//       },
//       bowlerOne: {
//         name: data.bowlerOne || '',
...
//         wickets: data.bowlerTwoWicket || '0',
...
//   /**
//    * Calculate prize pool based on match status
//    */
//   static calculatePrizePool(status) {
//     switch (status) {
//       case 'live': return 200000;
//       case 'upcoming': return 150000;
//       case 'completed': return 50000;
//       default: return 100000;
//     }
//   }

//   /**
//    * Generate mock cricket players for team creation
//    */
//   static generateCricketPlayers(team1, team2) {
...
//         team: team1,
//         role: i === 0 ? 'Wicket-Keeper' : roles[i % 3],
//         credits: Math.floor(Math.random() * 4) + 7, // 7-11 credits
...
//         team: team2,
//         role: i === 0 ? 'Wicket-Keeper' : roles[i % 3],
//         credits: Math.floor(Math.random() * 4) + 7,
//         points: Math.floor(Math.random() * 100),
//       });
//     }
